...
void UI_Layout_DrawHeader(const char *title)
{
    ...
    Screen_DrawTextUtf8Fallback(UI_HEADER_TEXT_X, (uint16_t)(UI_SAFE_Y + 1U), title, UI_ACCENT_COLOR, UI_BG_COLOR);
}
...
void UI_Layout_DrawFooter(const char *hint)
{
    ...
    {
        Screen_DrawTextUtf8Fallback(UI_FOOTER_TEXT_X, (uint16_t)(UI_SCREEN_H - UI_BOTTOM_H + 1U), hint, UI_FG_COLOR, UI_BG_COLOR);
    }
}

void UI_Layout_DrawTextLine(uint16_t x, uint16_t line, const char *text, uint16_t color)
{
    Screen_DrawTextUtf8Fallback(x, (uint16_t)(line * UI_LINE_HEIGHT), text, color, UI_BG_COLOR);
}
